...
impl <'a> FieldInfoCollection<'a> {
    pub fn all_fields(&self) -> Vec<&FieldInfo<'a>>{
        match self {
            ...
            FieldInfoCollection::EnumFields { variant_fields } => variant_fields.values().flatten().collect(),
        }
        
    }

    pub fn fields_by_variant_ident(&self, variant: syn::Ident) -> Option<&Vec<FieldInfo>> { // TODO: Change to Error
        match self {
